...
export default function Header() {
  ...
  return (
    <>
      {/* FIXED HEADER */}
      <header className="fixed top-0 left-0 w-full z-50 bg-bg/90 backdrop-blur border-b border-black/10">
        ...
        {/* MOBILE MENU */}
        {open && (
          <nav className="md:hidden px-6 pb-6 flex flex-col gap-4 text-xs tracking-widest uppercase">
            {navItems.map((item) => (
              <NavLink
                key={item}
                to={item === "home" ? "/" : `/${item}`}
